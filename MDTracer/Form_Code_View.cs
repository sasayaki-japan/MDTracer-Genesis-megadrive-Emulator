...
namespace MDTracer
{
    public partial class Form_Code_Trace
    {
            ...
            , int in_line_num, int in_top_line, int in_stop_line, int in_cursole_line, int in_hScrollBar)
        {
            ...
            e.Graphics.DrawString("mnemonic", wfont, wbrush, new PointF(240 - in_hScrollBar, 1));
            e.Graphics.DrawString("comment1", wfont, wbrush, new PointF(750 - in_hScrollBar, 1));
            //-----------------------------------------------------------
            string w_text;
            ...
            for (int w_cur = 0; w_cur < in_line_num; w_cur++)
            {
                ...
                TRACECODE w_code = g_analyse_code[w_cur_line];
                ...
                {
                    w_text = g_analyse_code[w_cur_line + i].val.ToString("X4");
                    e.Graphics.DrawString(w_text, wfont_data, Brushes.Blue, new PointF(70 + i * 30 - in_hScrollBar, 22 + w_cur * 16));
                }
                ...
                if (w_code.type == TRACECODE.TYPE.OPC)
                {
                    ...
                    e.Graphics.DrawString(w_text, wfont, Brushes.Black, new PointF(270 - in_hScrollBar, 20 + w_cur * 16));
                }
                //comment1
                e.Graphics.DrawString(w_code.comment1, wfont, Brushes.Black, new PointF(750 - in_hScrollBar, 20 + w_cur * 16));
